...
$q = "SELECT * FROM attendance_history, attendance_codes, studentbio, 
	...
	(attendance_history.attendance_history_student = studentbio.studentbio_id)
	...
	AND (attendance_history.attendance_history_date = '$start_db_date')";
...
<?
if(is_array($r)) {
	echo"<table align='center' width='80%' cellpadding=5><th align='center'>" . _REPORT_ATTENDANCE_TITLE . "$start_date</th>";
	foreach($r as $s) {
		...
		if($s->attendance_history_notes && $s->attendance_history_notes != '')
			echo"<tr><td><table width='100%'><tr><td class='tblhead' width='10%'><? echo _REPORT_ATTENDANCE_NOTES?>: </td>
				<td class='tblcont' width='90%' align='left'>$s->attendance_history_notes</td></tr></table></td></tr>";
		//the static column headers
		echo"<tr><td><table border=1 width='100%'><tr class='tblhead'><td><? echo _REPORT_ATTENDANCE_INTERNAL?></td><td><? echo _REPORT_ATTENDANCE_DOB?></td><td><? echo _REPORT_ATTENDANCE_SCHOOL?></td>";
